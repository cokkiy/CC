...
//
    ...
    m_editActions->addAction(formWindowManager->action(QDesignerFormWindowManagerInterface::DownAction));
    m_editActions->addAction(formWindowManager->action(QDesignerFormWindowManagerInterface::UpAction));
    m_editActions->addAction(formWindowManager->action(QDesignerFormWindowManagerInterface::RaiseAction));
    // 显示在自定义工具栏的配置项中,实现可配置,andrew,20150128
    ...
    formWindowManager->action(QDesignerFormWindowManagerInterface::DownAction)->setProperty(QDesignerActions::defaultToolbarPropertyName, true);
    formWindowManager->action(QDesignerFormWindowManagerInterface::UpAction)->setProperty(QDesignerActions::defaultToolbarPropertyName, true);
...
//
    ...
    foreach (QObject *plugin, builtinPlugins) {
        ...
        //关闭其它编辑模式，只保留信号槽的编辑模式,andrew,20150327
        break;
    }
...
//
    ...
    actDesigerMode = new QAction(tr("&Designer Mode"),this);
    actDesigerMode->setShortcut(Qt::CTRL + Qt::Key_D);
    actDesigerMode->setCheckable(true);
    connect(actDesigerMode, SIGNAL(triggered()), this, SLOT(setDesingerMode()));
    m_settingsActions->addAction(actDesigerMode);

    actRealTimeMode = new QAction(tr("&RealTime Mode"),this);
    actRealTimeMode->setShortcut(Qt::CTRL + Qt::Key_R);
    actRealTimeMode->setCheckable(true);
    connect(actRealTimeMode, SIGNAL(triggered()), this, SLOT(setRealTimeMode()));
    m_settingsActions->addAction(actRealTimeMode);

    m_settingsActions->addAction(createSeparator(this));

    actFullScreen = new QAction(tr("Full Screen"),this);
    //这里不启用空格的快捷方式,andrew
    //actFullScreen->setShortcut(QKeySequence(Qt::Key_Space));
    connect(actFullScreen, SIGNAL(triggered()), this, SLOT(setFullScreen()));
    m_settingsActions->addAction(actFullScreen);

    actOpenPrevPage = new QAction(tr("Open Prev Page"),this);
    //actOpenPrevPage->setShortcut(QKeySequence(Qt::Key_PageUp));
    connect(actOpenPrevPage, SIGNAL(triggered()), this, SLOT(slotOpenPrevPage()));
    m_settingsActions->addAction(actOpenPrevPage);

    actOpenNextPage = new QAction(tr("Open Next Page"),this);
    //actOpenNextPage->setShortcut(QKeySequence(Qt::Key_PageDown));
    connect(actOpenNextPage, SIGNAL(triggered()), this, SLOT(slotOpenNextPage()));
    m_settingsActions->addAction(actOpenNextPage);
...
QActionGroup *QDesignerActions::createHelpActions()
{
    ...
    QAction *mainHelpAction = new QAction(tr("指显软件帮助"), this);
...
//    helpActions->addAction(aboutPluginsAction);
    ...
    QAction *aboutDesignerAction = new QAction(tr("关于指显软件"), this);
...
void QDesignerActions::setAppMode(AppMode m)
{
    switch (m) {
    case DesignerMode:
        setDesingerMode();
...
void QDesignerActions::newForm(const QString &fileName)
{
    showNewFormDialog(fileName);
}
...
void QDesignerActions::setDesingerMode()
{
    m_workbench->setAppMode(DesignerMode);
    m_workbench->reopenPage();
    m_workbench->update();
    m_saveFormAction->setEnabled(true);
    m_saveFormAction->setVisible(true);
    m_saveAllFormsAction->setEnabled(true);
    m_saveAllFormsAction->setVisible(true);
    m_saveFormAsTemplateAction->setEnabled(true);
    m_saveFormAsTemplateAction->setVisible(true);
    m_savePreviewImageAction->setEnabled(false);
    m_savePreviewImageAction->setVisible(false);
    m_printPreviewAction->setEnabled(false);
    m_printPreviewAction->setVisible(false);
    actDesigerMode->setChecked(m_workbench->appmode()==DesignerMode?true:false);
    actRealTimeMode->setChecked(m_workbench->appmode()==RealTimeMode?true:false);
}

void QDesignerActions::setRealTimeMode()
{
    m_workbench->setAppMode(RealTimeMode);
    m_workbench->reopenPage();
    m_workbench->update();
    m_saveFormAction->setEnabled(false);
    m_saveFormAction->setVisible(false);
    m_saveAllFormsAction->setEnabled(false);
    m_saveAllFormsAction->setVisible(false);
    m_saveFormAsTemplateAction->setEnabled(false);
    m_saveFormAsTemplateAction->setVisible(false);
    m_savePreviewImageAction->setEnabled(true);
    m_savePreviewImageAction->setVisible(true);
    m_printPreviewAction->setEnabled(true);
    m_printPreviewAction->setVisible(true);
    actDesigerMode->setChecked(m_workbench->appmode()==DesignerMode?true:false);
    actRealTimeMode->setChecked(m_workbench->appmode()==RealTimeMode?true:false);
}
void QDesignerActions:: setFullScreen()
{
     m_workbench->fullScreen();
}
...
void QDesignerActions::slotOpenNextPage()
{
    emit m_core->openNextPage();
}

void QDesignerActions::slotOpenPrevPage()
{
    emit m_core->openPrevPage();
}
