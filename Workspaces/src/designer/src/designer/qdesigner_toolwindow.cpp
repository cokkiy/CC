...
// -- ZXPageTreeBrowser 画面浏览工具窗口，刘裕贵，２０１４０８２5

static inline QWidget *createImageExplorer(QDesignerFormEditorInterface *core, QWidget *parent = 0)
{
    QDesignerImageExplorerInterface *widget = (QDesignerImageExplorerInterface *)QDesignerComponents::createImageExplorer(core, parent);
    core->setImageExplorer((QDesignerImageExplorerInterface *)widget);
    return widget;
}

class ZXPageTreeBrowserToolWindow: public QDesignerToolWindow
{
public:
    explicit ZXPageTreeBrowserToolWindow(QDesignerWorkbench *workbench);

    virtual QRect geometryHint() const;
};

ZXPageTreeBrowserToolWindow::ZXPageTreeBrowserToolWindow(QDesignerWorkbench *workbench) :
    QDesignerToolWindow(workbench,
                        createImageExplorer(workbench->core()),
                        QStringLiteral("qt_designer_imageexplorer"),
                        QDesignerToolWindow::tr("Image Explorer"),
                        QStringLiteral("__qt_image_explorer_tool_action"),
                        Qt::RightDockWidgetArea)
{
}

QRect ZXPageTreeBrowserToolWindow::geometryHint() const
{
    const QRect g = availableToolWindowGeometry();
    const int margin = workbench()->marginHint();
                                                                   ...
                                                                   QDesignerWorkbench *workbench)
{
    ...
    case ZXPageTreeBrowser:
        return new ZXPageTreeBrowserToolWindow(workbench);
    default:
        break;
    }
    return 0;
