...
//      break;
    case QDialogButtonBox::AcceptRole: {
        ...
        if (openTemplate(&errorMessage)) {            
            accept();
            qDebug() << tr("Create Page:") << m_fileName;
            emit m_workbench->core()->callbackCreatePage(m_fileName,m_fileName+".ui");
        }  else {
...
{
    //解决由模板创建画面时，资源文件的路径不能正确更动的问题,andrew,20150304
    const QString templateFile = m_newFormWidget->currentTemplateFileName();
    m_fileName = m_newFormWidget->pageName();
    TaskInterface * task = NetComponents::getTastCenter();
    return m_workbench->openTemplate(templateFile, task->currentTaskPath() + g_pageRelativeFilePath + m_fileName+".ui", ptrToErrorMessage);
