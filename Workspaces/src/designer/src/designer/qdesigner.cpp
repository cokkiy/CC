      ...
      m_workbench(0), m_suppressNewFormShow(false)
{
    ...
    //连接定时器事件
    connect(t_timer, SIGNAL(timeout()), this, SLOT(timerEvent_r()));

    //启动定时器 10ms 各种信号周期需要为10ms的整数倍
    t_timer->start(10);
}

void QDesigner::timerEvent_r()
{
    ti->timerEvent_r();
}
...
void QDesigner::initialize()
{
    ...
    // 设置翻译路径，加载并安装翻译文件，andrew,20150827
    QStringList files;
    QString resourceDir = QCoreApplication::applicationDirPath();
    resourceDir += QDir::separator();
    resourceDir += QStringLiteral("..");
    resourceDir += QDir::separator();
    resourceDir += QStringLiteral("translations");
    resourceDir = QDir::cleanPath(resourceDir);
