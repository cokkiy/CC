...
void QDesignerSettings::setAppMode(AppMode mode)
{
    QDesignerSettingsInterface *s = settings();
...
AppMode QDesignerSettings::appMode() const
{
    const AppMode defaultMode = DesignerMode;
    AppMode appMode = static_cast<AppMode>(value(QStringLiteral("App/currentMode"), defaultMode).toInt());
    return appMode;
}
