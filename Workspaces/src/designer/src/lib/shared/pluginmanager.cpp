...
QStringList QDesignerPluginManager::defaultPluginPaths()
{
    ...
    plugindir += QStringLiteral("..");
    plugindir += QDir::separator();
    plugindir += QStringLiteral("plugins");
    plugindir += QDir::separator();
    plugindir += QStringLiteral("designer");
    plugindir = QDir::cleanPath(plugindir);
    result.append(plugindir);
    return result;
...
//    }
    // comment for no use,andrew,20150313
    ...
    m_d(new QDesignerPluginManagerPrivate(core))
{
    ...
    //andrew 20150303
    const QSettings settings(qApp->applicationDirPath()+g_settingRelativeFileName, QSettings::Format::IniFormat);
...
{
    //andrew 20150303
    QSettings settings(qApp->applicationDirPath()+g_settingRelativeFileName, QSettings::Format::IniFormat);
