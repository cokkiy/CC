...
//And New Page Class,andrew,20130303
#include <QtDesigner/QPage>
...
WidgetFactory::Strings::Strings() :
    ...
    m_abstractpage(QStringLiteral("AbstractPage"))
{
}
// ---------------- WidgetFactory
...
QWidget*  WidgetFactory::createCustomWidget(const QString &className, QWidget *parentWidget, bool *creationError) const
{
    *creationError = false;
    ...
    // 关闭类名和实际类名不一致的对话框式的告警信息,andrew,20150828
...
QWidget *WidgetFactory::createWidget(const QString &widgetName, QWidget *parentWidget) const
{
    ...
    do {
        ...
        } else if (widgetName == m_strings.m_abstractpage) {
            //新建画面对象,andrew,20150303
            w = new AbstractPage(parentWidget);
