        ...
        const QString &propertyName, bool isMainContainer)
{
   ...
   if (hit != stringPropertyTypes().constEnd())
    ...
    // dataes: 设置多参数数据的配置类别,andrew,20140421
    if (propertyName.endsWith(QStringLiteral("dataes"))) {
        return StringPropertyParameters(ValidationDataes, true);
    }

    // files: 设置选择文件的配置类别,rika,20150814
    if (propertyName == QStringLiteral("file")) {
        return StringPropertyParameters(ValidationFile, true);
    }
