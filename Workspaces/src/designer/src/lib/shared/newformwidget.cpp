...
{
    { QT_TRANSLATE_NOOP("qdesigner_internal::NewFormWidget", "通用分辨率(1280x1024)"), 1280, 1024 },
    { QT_TRANSLATE_NOOP("qdesigner_internal::NewFormWidget", "发射站分辨率(1024x768)"), 1024, 768 }
};
...
//    }
    ...
    // Additional default template paths
    const QStringList defaultformTemplatePaths = settings.defaultFormTemplatePaths();
    const QStringList::const_iterator ftcend = defaultformTemplatePaths.constEnd();
    for (QStringList::const_iterator it = defaultformTemplatePaths.constBegin(); it != ftcend; ++it)
...
//        loadFrom(*it, faformTemplatelse, uiExtension, formTemplate, selectedItem);
...
//    }
    ...
    // Fill profile combo
    m_ui->profileComboBox->setText("PageName");
...
QString NewFormWidget::currentTemplateFileName()
{
    const QVariant templateFileName = m_currentItem->data(0, TemplateNameRole);
    if (templateFileName.type() == QVariant::String)
        return templateFileName.toString();
    return QString::fromUtf8("");
}
QString NewFormWidget::pageName()
{
    return m_ui->profileComboBox->text();
}
}
