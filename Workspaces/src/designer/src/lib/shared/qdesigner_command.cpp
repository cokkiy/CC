...
// ---- DownWidgetCommand ----
DownWidgetCommand::DownWidgetCommand(QDesignerFormWindowInterface *formWindow)
    : ChangeZOrderCommand(formWindow)
{
}

QWidgetList DownWidgetCommand::reorderWidget(const QWidgetList &list, QWidget *widget) const
{
    QWidgetList l = list;
    const int index = l.indexOf(widget);
    if (index-1 != -1 && index < l.count())
    {
        l.removeAll(widget);
        l.insert(index-1,widget);
    }
    return l;
}

void DownWidgetCommand::init(QWidget *widget)
{
    ChangeZOrderCommand::init(widget);
    setText(QApplication::translate("Command", "Low '%1'").arg(widget->objectName()));
    const int index = m_oldParentZOrder.indexOf(m_widget);
    if (index - 1 != -1 && index < m_oldParentZOrder.count())
        m_oldAfter = m_oldParentZOrder.at(index - 1);
}

void DownWidgetCommand::reorder(QWidget *widget) const
{
    if(m_oldAfter)
        widget->stackUnder(m_oldAfter);
    else
        widget->lower();
}

// ---- UpWidgetCommand ----
UpWidgetCommand::UpWidgetCommand(QDesignerFormWindowInterface *formWindow)
    : ChangeZOrderCommand(formWindow)
{
}

QWidgetList UpWidgetCommand::reorderWidget(const QWidgetList &list, QWidget *widget) const
{
    QWidgetList l = list;
    const int index = l.indexOf(widget);
    if (index != -1 && index + 1 < l.count())
    {
        l.removeAll(widget);
        l.insert(index+1,widget);
    }
...
void UpWidgetCommand::init(QWidget *widget)
{
    ChangeZOrderCommand::init(widget);
    setText(QApplication::translate("Command", "Low '%1'").arg(widget->objectName()));
    const int index = m_oldParentZOrder.indexOf(m_widget);
    if (index != -1 && index+2 < m_oldParentZOrder.count())
        m_oldBefore = m_oldParentZOrder.at(index + 2);
}

void UpWidgetCommand::reorder(QWidget *widget) const
{
    if(m_oldBefore)
        widget->stackUnder(m_oldBefore);
    else
        widget->raise();
}
