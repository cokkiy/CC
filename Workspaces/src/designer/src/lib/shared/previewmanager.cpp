                                       ...
                                       int initialZoom)
{
    ...
    if (!formWidget)
        return 0;
    //直接返回创建的实时指显画面的窗口,andrew,20150917
    return formWidget;
...
{
    enum { Spacing = 10 };
    // 已经存在某预览窗口，则直接返回该窗口,andrew,20150129
...
//    }
    ...
    //widget->setMaximumSize(QSize(QApplication::desktop()->width(),QApplication::desktop()->height()));
    //widget->showFullScreen();//show();
