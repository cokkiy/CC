...
#include <qdesigner_imageexplorer_p.h>
...
void FormWindowManager::addFormWindow(QDesignerFormWindowInterface *w)
{
    ...
    if (QDesignerImageExplorer *ie = qobject_cast<QDesignerImageExplorer *>(m_core->imageExplorer()))
        connect(w, SIGNAL(mainContainerChanged(QWidget*)), ie, SLOT(mainContainerChanged()));
...
void FormWindowManager::setupActions()
{
    ...
    m_actionDown = new QAction(createIconSet(QStringLiteral("editlower.png")), tr("Send to &Down"), this);
    m_actionDown->setObjectName(QStringLiteral("__qt_down_action"));
    m_actionDown->setShortcut(Qt::CTRL + Qt::Key_D);
    m_actionDown->setStatusTip(tr("Down the selected widgets"));
    m_actionDown->setWhatsThis(tr("Down the selected widgets"));
    connect(m_actionDown, SIGNAL(triggered()), this, SLOT(slotActionDownActivated()));
    m_actionDown->setEnabled(false);

    m_actionUp = new QAction(createIconSet(QStringLiteral("editraise.png")), tr("Send to &Up"), this);
    m_actionUp->setObjectName(QStringLiteral("__qt_up_action"));
    m_actionUp->setShortcut(Qt::CTRL + Qt::Key_U);
    m_actionUp->setStatusTip(tr("Up the selected widgets"));
    m_actionUp->setWhatsThis(tr("Up the selected widgets"));
    connect(m_actionUp, SIGNAL(triggered()), this, SLOT(slotActionUpActivated()));
    ...
    m_newFormAction = new QAction(tr("&New..."), this);
...
void FormWindowManager::createForm()
{
    showNewFormDialog(QString());
}

void FormWindowManager::showNewFormDialog(const QString &fileName)
{
    closeAllPreviews();
//    if()
...
void FormWindowManager::slotActionDownActivated()
{
    m_activeFormWindow->downWidgets();
}

void FormWindowManager::slotActionUpActivated()
{
    m_activeFormWindow->upWidgets();
}
...
void FormWindowManager::slotUpdateActions()
{
    ...
    m_actionDown->setEnabled(canChangeZOrder && selectedWidgetCount > 0);
    m_actionUp->setEnabled(canChangeZOrder && selectedWidgetCount > 0);
...
QAction *FormWindowManager::action(Action action) const
{
    ...
    case QDesignerFormWindowManagerInterface::DownAction:
        return m_actionDown;
    case QDesignerFormWindowManagerInterface::UpAction:
        return m_actionUp;
