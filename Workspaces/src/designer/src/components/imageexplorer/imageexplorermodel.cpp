...
static qdesigner_internal::ImageData::StandardItemList createModelRow(const QObject *o)
{
    qdesigner_internal::ImageData::StandardItemList rc;
    const Qt::ItemFlags baseFlags = Qt::ItemIsSelectable|Qt::ItemIsDropEnabled|Qt::ItemIsEnabled;
    for (int i = 0; i < qdesigner_internal::ImageExplorerModel::NumColumns; i++) {
        QStandardItem *item = new QStandardItem;
        Qt::ItemFlags flags = baseFlags;
        if (i == qdesigner_internal::ImageExplorerModel::ObjectNameColumn && isNameColumnEditable(o))
...
namespace qdesigner_internal {
    ...
    struct ModelRecursionContext_Image {
        explicit ModelRecursionContext_Image(QDesignerFormEditorInterface *core, const QString &sepName);
    ...
    ModelRecursionContext_Image::ModelRecursionContext_Image(QDesignerFormEditorInterface *c, const QString &sepName) :
    ...
    ImageData::ImageData(QObject *parent, QObject *object, const ModelRecursionContext_Image &ctx) :
       m_parent(parent),
       m_object(object),
       m_type(Object),
    ...
    void ImageData::initObject(const ModelRecursionContext_Image &ctx)
    {
    ...
    void ImageData::initWidget(QWidget *w, const ModelRecursionContext_Image &ctx)
    {
    ...
    bool ImageData::equals(const ImageData & me) const
    {
        return m_parent == me.m_parent && m_object == me.m_object;
    }

    unsigned ImageData::compare(const ImageData & rhs) const
    {
        unsigned rc = 0;
    ...
    void ImageData::setItemsDisplayData(const StandardItemList &row, const ImageExplorerIcons &icons, unsigned mask) const
    {
        if (mask & ObjectNameChanged)
            row[ImageExplorerModel::ObjectNameColumn]->setText(m_objectName);
        if (mask & ClassNameChanged) {
            row[ImageExplorerModel::ClassNameColumn]->setText(m_className);
            row[ImageExplorerModel::ClassNameColumn]->setToolTip(m_className);
        }
        ...
        if (mask & (ClassIconChanged|TypeChanged|LayoutTypeChanged)) {
            switch (m_type) {
            case LayoutWidget:
                row[ImageExplorerModel::ObjectNameColumn]->setIcon(icons.layoutIcons[m_managedLayoutType]);
                row[ImageExplorerModel::ClassNameColumn]->setIcon(icons.layoutIcons[m_managedLayoutType]);
                break;
            case LayoutableContainer:
                row[ImageExplorerModel::ObjectNameColumn]->setIcon(icons.layoutIcons[m_managedLayoutType]);
                row[ImageExplorerModel::ClassNameColumn]->setIcon(m_classIcon);
                break;
            default:
                row[ImageExplorerModel::ObjectNameColumn]->setIcon(QIcon());
                row[ImageExplorerModel::ClassNameColumn]->setIcon(m_classIcon);
                break;
            }
        }
    ...
    void ImageData::setItems(const StandardItemList &row, const ImageExplorerIcons &icons) const
    {
        const QVariant object = QVariant::fromValue(m_object);
        row[ImageExplorerModel::ObjectNameColumn]->setData(object, DataRole);
        row[ImageExplorerModel::ClassNameColumn]->setData(object, DataRole);
    ...
    void createModelRecursion(const QDesignerFormWindowInterface *fwi,
                              ...
                              QObject *object,
                              ImageModel &model,
                              const ModelRecursionContext_Image &ctx)
    {
        typedef QList<QButtonGroup *> ButtonGroupList;
        typedef QList<QAction *> ActionList;

        // 1) Create entry
        const ImageData entry(parent, object, ctx);
        ...
        if (entry.type() == ImageData::ExtensionContainer) {
    ...
    ImageExplorerModel::ImageExplorerModel(QObject *parent) :
    ...
    void ImageExplorerModel::clearItems()
    {
        beginResetModel();
    ...
    ImageExplorerModel::UpdateResult ImageExplorerModel::update(QDesignerFormWindowInterface *fw)
    {
        ...
        const ModelRecursionContext_Image ctx(fw->core(),  separator);
    ...
    QObject *ImageExplorerModel::objectAt(const QModelIndex &index) const
    {
        if (index.isValid())
    ...
    ImageExplorerModel::StandardItemList ImageExplorerModel::rowAt(QModelIndex index) const
    {
        StandardItemList rc;
        while (true) {
    ...
    void ImageExplorerModel::rebuild(const ImageModel &newModel)
    {
        clearItems();
        if (newModel.empty())
            return;

        const ImageModel::const_iterator mcend = newModel.constEnd();
        ImageModel::const_iterator it = newModel.constBegin();
        // Set up root element
    ...
    void ImageExplorerModel::updateItemContents(ImageModel &oldModel, const ImageModel &newModel)
    {
        ...
        for (int i = 0; i < size; i++) {
            const ImageData &newEntry = newModel[i];
            ImageData &entry =  oldModel[i];
    ...
    QVariant ImageExplorerModel::data(const QModelIndex &index, int role) const
    {
    ...
    bool ImageExplorerModel::setData(const QModelIndex &index, const QVariant &value, int role)
    {
