...
#include <imageexplorer/imageexplorer.h>
...
QDesignerWidgetBoxInterface *QDesignerComponents::createWidgetBox(QDesignerFormEditorInterface *core, QWidget *parent)
{
    ...
    //屏蔽从用户的插件配置文件中读取插件信息的代码,andrew,20150817
...
/*!
    Returns a new Image Explorer interface with the given \a parent for the \a core interface.*/
QDesignerImageExplorerInterface *QDesignerComponents::createImageExplorer(QDesignerFormEditorInterface *core, QWidget *parent)
{
    qdesigner_internal::ImageExplorer *  imageExplorer = new qdesigner_internal::ImageExplorer(core, parent);
    TaskInterface * t_task = NetComponents::getTastCenter();
    QString t_taskpath = t_task->currentTaskPath();
    imageExplorer->setFileName(t_taskpath + g_bookmarkRelativeFilePath);
    imageExplorer->load();
