...
- [ ] cc-cpu-monitor插件
- [ ] cc-memory-monitor插件
- [ ] cc-network-monitor插件
- [ ] cc-process-monitor插件
- [ ] cc-disk-monitor插件
...
### 3.1 Plugin Host实现
**任务:**
- [ ] PluginManager.ts插件加载/卸载/生命周期
- [ ] PanelRegistry.ts UI区域注册表
- [ ] ActionRegistry.ts 控制命令注册表
- [ ] DataBridge.ts WS数据→Store桥接

### 3.2 配置驱动UI
**任务:**
- [ ] 定义layout.json Schema
- [ ] DynamicPanel.tsx动态渲染组件
- [ ] AlertOverlay.tsx告警覆盖层
...
- 2026-04-19 10:00: Phase 1.3-1.4 核心功能完成 (Codex+Claude)
- 2026-04-19 11:00: Phase 2-3 尚未开始，Phase 4 阻塞(MQTT Broker缺失)
...
| 问题 | 优先级 | 状态 | 说明 |
|------|--------|------|------|
| 缺少MQTT Broker | P0 | 阻塞中 | Mosquitto未安装，Docker拉取失败，NATS下载超时 |
| Phase 2 无人负责 | P1 | 待分配 | 需要分配开发资源 |
| Phase 3 无人负责 | P1 | 待分配 | 需要分配开发资源 |
