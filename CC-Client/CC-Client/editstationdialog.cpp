     ...
     : QDialog(parent)
{
    ...
    //设置输入验证
    macRegExp.setPattern(QStringLiteral("((\\d|[a-f]|[A-F]){2}-){7}(\\d|[a-f]|[A-F]){2}"));
    ipRegExp.setPattern(QStringLiteral("(((?:(?:25[0-5]|2[0-4]\\d|((1\\d{2})|([1-9]?\\d)))\\.){3}(?:25[0-5]|2[0-4]\\d|((1\\d{2})|([1-9]?\\d)))),)+"));
...
void EditStationDialog::on_removeNIPushButton_clicked()
{
    ...
    {
        QMessageBox::warning(NULL, QStringLiteral("错误"), QStringLiteral("没有可以删除的网卡设置项"));
    }
}
...
void EditStationDialog::on_OKPushButton_clicked()
{
    ...
    {
        QMessageBox::warning(NULL, QStringLiteral("错误"), QStringLiteral("工作站名称不能为空"));
        return;
    }
...
void EditStationDialog::on_addStartAppPushButton_clicked()
{
    ...
    {
        QMessageBox::warning(NULL, QStringLiteral("错误"), QStringLiteral("没有输入要启动程序名称."));
        return;
    }
    ...
    QTableWidgetItem *pathItem = new QTableWidgetItem(ui->appNameLineEdit->text());
...
void EditStationDialog::on_addMonitorProcPushButton_clicked()
{
    ...
    {
        QMessageBox::warning(NULL, QStringLiteral("错误"), QStringLiteral("没有输入要监视的进程名称."));
        return;
    }
...
void EditStationDialog::UpdateStation()
{
    ...
    {
        station->addStartApp(ui->startAppTableWidget->item(i, 0)->text(), ui->startAppTableWidget->item(i, 1)->text(),
            ui->startAppTableWidget->item(i, 2)->text(), ui->startAppTableWidget->item(i, 3)->checkState() == Qt::Checked);
    }
    station->clearStandAloneMonitorProc();
    for (int i = 0; i < ui->monitorListWidget->count(); i++)
    {
        station->addStandAloneMonitorProcess(ui->monitorListWidget->item(i)->text());
    }
    station->NetworkIntefaces.clear();
    QStringList ips = ui->NIIPLineEdit->text().split(",", QString::SkipEmptyParts);
...
void EditStationDialog::initUI()
{
    if (station != NULL)
    {
        ...
        {
            QTableWidgetItem* pathItem = new QTableWidgetItem(QString::fromStdString(app.AppPath));
            ui->startAppTableWidget->setItem(row, 0, pathItem);
            QTableWidgetItem* argItem = new QTableWidgetItem(QString::fromStdString(app.Arguments));
            ui->startAppTableWidget->setItem(row, 1, argItem);
            QTableWidgetItem* procNameItem = new QTableWidgetItem(QString::fromStdString(app.ProcessName));
            ui->startAppTableWidget->setItem(row, 2, procNameItem);
            QTableWidgetItem* allowMultiInstanceItem = new QTableWidgetItem();
            allowMultiInstanceItem->setCheckState(app.AllowMultiInstance ? Qt::Checked : Qt::Unchecked);
            ui->startAppTableWidget->setItem(row, 3, allowMultiInstanceItem);
            row++;
        }
