...
namespace CC_StationService
{
    ...
    public static class WeatherImageHelper
    {
        ...
        public static PictureType CheckType(string filename)
        {
            ...
            if (Regex.IsMatch(filename, fy2DPattern))
                return PictureType.FY2D;
            if (Regex.IsMatch(filename, fy2EPattern))
                return PictureType.FY2E;
            if (Regex.IsMatch(filename, fy2GPattern))
                return PictureType.FY2G;
            if (Regex.IsMatch(filename, wdPlainGraphPattern))
                return PictureType.WDRadarPlainGraph;
            if (Regex.IsMatch(filename, wdVerticalSweepPattern))
                return PictureType.WDRadarVerticalSweep;
            if (Regex.IsMatch(filename, dslPlainGraphPattern))
                return PictureType.DSLRadarPlainGraph;
            if (Regex.IsMatch(filename, dslVerticalSweepPattern))
                return PictureType.DSLRadarVerticalSweep;
            if (Regex.IsMatch(filename, dslVelocitySpectrumPattern))
