...
@Injectable()
export class OrgTracePublishGuard implements CanActivate, CanActivateChild, CanLoad {
    ...
    checkLogin(url: string): boolean {
        ...
        if (this.authService.loggedIn) {
            ...
            if (token["role.Mode"]) {
                var regex = new RegExp("^(Manage|Publish).[{(]?[0-9A-F]{8}[-]?([0-9A-F]{4}[-]?){3}[0-9A-F]{12}[)}]?$", "i");
                var index = token["role.Mode"].findIndex((m: string) => { return regex.test(m); });
