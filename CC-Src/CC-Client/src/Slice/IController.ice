...
{	
	...
	interface IController
	{
		...
		//重命名文件
		bool renameFile(["cpp:type:wstring"]string oldName,["cpp:type:wstring"]string newName);
	...
	{
		///创建文件		
		bool createFile(["cpp:type:wstring"]string fileName) throws FileTransException;
		///传送数据
		...
		///获取文件大小
		long getSize(["cpp:type:wstring"]string fileName) throws FileTransException;

		///获取文件数据
		/*
		* return：当返回false时，文件尚未读取完，返回treu时，文件读取完毕
		*/
		bool getData(["cpp:type:wstring"]string fileName,long position,out int length,out ByteArray data) throws FileTransException;
	};
};