...
*/
StationTableModel::StationTableModel(const StationList *pStations,
                                     QObject *parent)
    : QAbstractTableModel(parent) {
  this->pStations = pStations;
  const_cast<StationList *>(pStations)
      ->subscribeAllStationsPropertyChangedEvent(
          this, SLOT(stationPropertyChanged(const QString &, const QObject *)));
  connect(pStations, SIGNAL(stationListChanged()), this,
          SLOT(stationListChanged()));
  connect(pStations, SIGNAL(stationAdded(StationInfo *)), this,
          SLOT(stationAdded(StationInfo *)));
...
//返回行数
int StationTableModel::rowCount(
    const QModelIndex &parent /*= QModelIndex()*/) const {
  return pStations->currentCount();
}
//返回列数
int StationTableModel::columnCount(
    const QModelIndex &parent /*= QModelIndex()*/) const {
  return totalColumnCount;
}
// view 获取数据方法
QVariant StationTableModel::data(const QModelIndex &index,
  ...
  if (role == Qt::DisplayRole) {
    if (displayMode == List) {
      QString str =
          QStringLiteral("名称：%1\tIP：%2\t状态：%3\t内存(%)：%4\tCPU：%5");
      StationInfo *s = pStations->atCurrent(index.row());
      QString memory = QStringLiteral("%1").arg(
          pStations->atCurrent(index.row())->Memory() /
              pStations->atCurrent(index.row())->TotalMemory() * 100,
          0, 'f', 2);
      str = str.arg(s->Name())
                .arg(s->IP())
                .arg(s->state().toString())
                .arg(memory)
                .arg(s->CPU(), 0, 'f', 1);
  ...
  if (role == Qt::TextAlignmentRole) {
    if (displayMode == LargerIcons || displayMode == SmallIcons) {
      ...
      return 0x41; // Qt::AlignLeft | Qt::AlignBottom;
    }
  }
...
void StationTableModel::sort(int column,
                             Qt::SortOrder order /*= Qt::AscendingOrder*/) {}

void StationTableModel::beginFilter() { beginResetModel(); }

void StationTableModel::endFilter() { endResetModel(); }

// view获取行标题和列标题
QVariant StationTableModel::headerData(int section, Qt::Orientation orientation,
                                       int role /*= Qt::DisplayRole*/) const {
  if (role != Qt::DisplayRole)
    return QVariant();
...
*/
void StationTableModel::setDisplayMode(
    DisplayMode mode /*= DisplayMode::SmallIcons*/) {
...
QVariant StationTableModel::getIcon(const QModelIndex &index) const {
  ...
  if (displayMode == LargerIcons) {
    ...
    case StationInfo::Unknown:
      icon.addFile(QStringLiteral(":/Icons/ncom002.ico"), QSize(64, 64),
                   QIcon::Normal, QIcon::On);
      break;
    case StationInfo::Normal:
      if (pStations->atCurrent(index.row())->NeedUpdate()) {
        icon.addFile(QStringLiteral(":/Icons/ncom003-up.png"), QSize(64, 64),
                     QIcon::Normal, QIcon::On);
      } else {
        icon.addFile(QStringLiteral(":/Icons/ncom003.png"), QSize(64, 64),
                     QIcon::Normal, QIcon::On);
      }
      break;
    case StationInfo::Warning:
      if (pStations->atCurrent(index.row())->NeedUpdate()) {
        icon.addFile(QStringLiteral(":/Icons/ncom008-up.png"), QSize(64, 64),
                     QIcon::Normal, QIcon::On);
      } else {
        icon.addFile(QStringLiteral(":/Icons/ncom008.ico"), QSize(64, 64),
                     QIcon::Normal, QIcon::On);
      }
      break;
    case StationInfo::Error:
      icon.addFile(QStringLiteral(":/Icons/ncom006.ico"), QSize(64, 64),
                   QIcon::Normal, QIcon::On);
      break;
    case StationInfo::WarningBlocked:
      icon.addFile(QStringLiteral(":/Icons/warningblocked.png"), QSize(64, 64),
                   QIcon::Normal, QIcon::On);
      break;
    case StationInfo::NormalBlocked:
      icon.addFile(QStringLiteral(":/Icons/normalblocked.png"), QSize(64, 64),
                   QIcon::Normal, QIcon::On);
      break;
    }
    switch (pStations->atCurrent(index.row())->state().getOperatingStatus()) {
    case StationInfo::Powering:
      return powerOnIcons[pStations->atCurrent(index.row())->ExecuteCounting() %
                          4];
      break;
    case StationInfo::AppStarting:
      return startCloseAppIcons
          [pStations->atCurrent(index.row())->ExecuteCounting() % 2];
      break;
    case StationInfo::Rebooting:
    case StationInfo::Shutdowning:
      return shutdownRebootIcons
          [pStations->atCurrent(index.row())->ExecuteCounting() % 5];
      break;
    }
    v.setValue(icon);
  } else {
    ...
    switch (pStations->atCurrent(index.row())->state().getRunningState()) {
    case StationInfo::Unknown:
      icon.addFile(QStringLiteral(":/Icons/ncom002.ico"), QSize(48, 48),
                   QIcon::Normal, QIcon::On);
      break;
    case StationInfo::Normal:
      if (pStations->atCurrent(index.row())->NeedUpdate()) {
        icon.addFile(QStringLiteral(":/Icons/ncom003-up.png"), QSize(48, 48),
                     QIcon::Normal, QIcon::On);
      } else {
        icon.addFile(QStringLiteral(":/Icons/ncom003.png"), QSize(48, 48),
                     QIcon::Normal, QIcon::On);
      }
      break;
    case StationInfo::Warning:
      if (pStations->atCurrent(index.row())->NeedUpdate()) {
        icon.addFile(QStringLiteral(":/Icons/ncom008-up.png"), QSize(48, 48),
                     QIcon::Normal, QIcon::On);
      } else {
        icon.addFile(QStringLiteral(":/Icons/ncom008.ico"), QSize(48, 48),
                     QIcon::Normal, QIcon::On);
      }
      break;
    case StationInfo::Error:
      icon.addFile(QStringLiteral(":/Icons/ncom006.ico"), QSize(48, 48),
                   QIcon::Normal, QIcon::On);
      break;
    case StationInfo::WarningBlocked:
      icon.addFile(QStringLiteral(":/Icons/warningblocked.png"), QSize(64, 64),
...
QVariant StationTableModel::getColumnValue(const QModelIndex &index) const {
  ...
  case 4:
    return QStringLiteral("%1").arg(s->Memory() / s->TotalMemory() * 100, 0,
                                    'f', 2);
  case 5: {
    QString receive = QStringLiteral("");
    for (auto &data : ifDatas) {
      QString r = QStringLiteral("%0: %1 ")
                      .arg(QString::fromStdString(data.IfName))
...
//工作站状态发生变化
void StationTableModel::stationPropertyChanged(const QString &propertyName,
                                               const QObject *station) {
...
void StationTableModel::stationAdded(StationInfo *addedStation) {
  addedStation->subscribePropertyChanged(
      this, SLOT(stationPropertyChanged(const QString &, const QObject *)));
}
