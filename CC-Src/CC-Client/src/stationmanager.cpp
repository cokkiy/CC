...
创建时间:2016/4/5 15:52:56
*/
void StationManager::block(bool blocked) {
  for (auto index : stationIndexs) {
    StationInfo *s = pStations->atCurrent(index.row());
    s->block(blocked);
  }
...
void StationManager::sendPowerOnPacket(StationInfo *s) {
  ...
  for (QString &mac : macs) {
    ...
    for (int i = 0; i < 5; i++) {
      udpSocket.writeDatagram(powerOnData.data(), powerOnData.size(),
...
//设置气象云图下载选项
void StationManager::setWeatherImageDownloadOption(
  ...
  for (auto &station : *pStations) {
    ...
    if (prx != NULL) {
      ...
      prx->begin_setWeatherPictureDownloadOption(
          ccOption, [&station, &ccOption]() {
  ...
  for (auto index : stationIndexs) {
    if (index.column() == 0) {
      ...
      if (s->controlProxy != NULL) {
        s->controlProxy->begin_openSpecPage(
            pageNum, []() {},
            [s](const Ice::Exception &ex) {
              const Ice::OperationNotExistException *pONE =
                  dynamic_cast<const Ice::OperationNotExistException *>(&ex);
              if (pONE != NULL) {
                QMessageBox::warning(
                    NULL, QStringLiteral("发生错误"),
                    QStringLiteral("工作站%1不支持该操作，需要升级中控服务。")
                        .arg(s->Name()));
              } else {
                QMessageBox::warning(NULL, QStringLiteral("发生错误"),
                                     QStringLiteral("%1").arg(ex.what()));
              }
            });
      }
    }
...
void StationManager::clearPage() {
  for (auto index : stationIndexs) {
    if (index.column() == 0) {
      StationInfo *s = pStations->atCurrent(index.row());
      if (s->controlProxy != NULL) {
        s->controlProxy->begin_switchDisplayPageAndMode(
            ClearPage, []() {},
...
void StationManager::clearPage(int subSystem) {
  ...
  for (auto index : stationIndexs) {
    if (index.column() == 0) {
      StationInfo *s = pStations->atCurrent(index.row());
      if (s->controlProxy != NULL) {
        s->controlProxy->begin_sendRemoteCtrlCommand(
            ClearSubSysPage, params, []() {},
            [](const Ice::Exception &ex) { auto w = ex.what(); });
      }
    }
  }
}
...
void StationManager::reboot(StationInfo *s) {
  ...
  if (prx != NULL) {
    ...
    try {
      ...
      s->setState(StationInfo::RebootFailure,
                  QStringLiteral("网络故障,连接通道已关闭"));
          ...
          [s](const AppStartingResults &results) {
            ...
            for (auto &result : results) {
              if (result.CtrlResult == FailToStart) {
                ok = false;
                message += QStringLiteral("%1启动失败 ")
                               .arg(s->getStartAppNameByIndex(result.ParamId));
              } else if (result.CtrlResult == AlreadyRunning) {
                message += QStringLiteral("%1已经在运行 ")
                               .arg(s->getStartAppNameByIndex(result.ParamId));
              }

              //如果自动监视启动进程
              if (result.CtrlResult == AlreadyRunning ||
                  result.CtrlResult == Started) {
                //添加自动监视进程信息
              }
            }
            if (ok) {
            ...
            try {
              ...
              s->setState(StationInfo::GeneralError,
                          QStringLiteral("设置工作站列表错误"));
            }
          },
          [s](const Ice::Exception &ex) {
            AppControlError *controlError = dynamic_cast<AppControlError *>(
                const_cast<Ice::Exception *>(&ex));
            if (NULL != controlError) {
              //应用程序启动代理没有启动
              s->setState(StationInfo::AppStartFailure,
                          QString::fromStdWString(controlError->Reason));
              return;
            }
            Ice::ConnectionRefusedException *refusedError =
                dynamic_cast<Ice::ConnectionRefusedException *>(
                    const_cast<Ice::Exception *>(&ex));
            if (NULL != refusedError) {
              //网络故障,无法连接到工作站监控服务程序
              s->setState(
                  StationInfo::AppStartFailure,
                  QStringLiteral("网络故障,无法连接到工作站监控服务程序"));
              return;
            }
            //其他未知错误
            s->setState(StationInfo::AppStartFailure,
...
void StationManager::exitApp(StationInfo *s) {
  ...
  if (processesId.empty()) {
    ...
    try {
      s->controlProxy->begin_closeApp(
          processesId,
          [s](const CC::AppControlResults &results) {
            for (auto &r : results) {
              if (r == FailToClose || r == Error) {
                s->setState(StationInfo::AppCloseFailure,
                            QStringLiteral("部分或全部程序无法退出."));
              }
            }
          },
          [s](const Ice::Exception &ex) {
