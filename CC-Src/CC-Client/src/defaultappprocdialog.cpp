...
DefaultAppProcDialog::DefaultAppProcDialog(Option &option, QString title,
                                           QWidget *parent)
    : QDialog(parent) {
  ...
  for (auto &param : this->option->StartApps) {
    QTableWidgetItem *pathItem =
        new QTableWidgetItem(QString::fromStdWString(param.AppPath));
    ui->appTableWidget->setItem(row, 0, pathItem);
    QTableWidgetItem *argItem =
        new QTableWidgetItem(QString::fromStdString(param.Arguments));
    ui->appTableWidget->setItem(row, 1, argItem);
    QTableWidgetItem *proceNameItem =
        new QTableWidgetItem(QString::fromStdString(param.ProcessName));
    ...
    allowMultiInstanceItem->setCheckState(
        param.AllowMultiInstance ? Qt::Checked : Qt::Unchecked);
    ui->appTableWidget->setItem(row, 3, allowMultiInstanceItem);
    row++;
  }
  ui->procListWidget->addItems(
      QStringList::fromStdList(this->option->MonitorProcesses));
}

DefaultAppProcDialog::~DefaultAppProcDialog() { delete ui; }
...
void DefaultAppProcDialog::on_addAppButton_clicked() {
  if (ui->appLineEdit->text() == QStringLiteral("")) {
    QMessageBox::warning(NULL, QStringLiteral("错误"),
                         QStringLiteral("必须填写需要启动程序完整路径和名称"));
    return;
  }
...
void DefaultAppProcDialog::on_OKPushButton_clicked() {
  ...
  for (int i = 0; i < ui->appTableWidget->rowCount(); i++) {
    ...
    param.AppPath = ui->appTableWidget->item(i, 0)->text().toStdWString();
    param.Arguments = ui->appTableWidget->item(i, 1)->text().toStdString();
    param.ProcessName = ui->appTableWidget->item(i, 2)->text().toStdString();
    param.AllowMultiInstance =
        ui->appTableWidget->item(i, 3)->checkState() == Qt::Checked;
