...
bool StationInfo::IsRunning() { return m_state != Unknown; }

bool StationInfo::NeedUpdate() { return m_NeedUpdate; }
...
QString StationInfo::Name() { return m_Name; }
...
int StationInfo::ExecuteCounting() { return m_ExecuteCounting; }

void StationInfo::setExecuteCounting(int value) {
  if (m_ExecuteCounting != value) {
    m_ExecuteCounting = value;
    m_state.setAttachMessage(
        QStringLiteral("命令已发送%1秒...").arg(m_ExecuteCounting));
    emit propertyChanged("ExecuteCounting", this);
  }
}

float StationInfo::AppMemory() { return m_ZXMemory; }
...
qint64 StationInfo::TotalMemory() { return m_TotalMemory; }
...
int StationInfo::AppThreadCount() { return m_zxThreadCount; }
...
int StationInfo::ProcCount() { return m_ProcCount; }
...
float StationInfo::AppCPU() { return m_zxCPU; }
...
float StationInfo::Memory() { return m_Memory; }
...
void StationInfo::setMemory(float value) {
  ...
  counterData["Idle"].memoryData[0] =
      100.0 - counterData["Total"].memoryData[0];
  if (m_Memory != value) {
    m_Memory = value;
...
StationInfo::State StationInfo::state() { return m_state; }
...
void StationInfo::block(bool blocked) { m_state.block(blocked); }

float StationInfo::CPU() { return m_CPU; }
...
void StationInfo::setLastTick(const time_t &tickTime /*= time(NULL)*/) {
  ...
  if (AppsRunningState.size() >= getShouldMonitoredProcessesCount()) {
    //结果和监视数量一致
    auto running =
        find_if(AppsRunningState.begin(), AppsRunningState.end(),
    ...
    if (notRunning == AppsRunningState.end()) {
      ...
      //部分运行
      setState(SomeAppNotRunning,
               QStringLiteral("详情可通过\"详细信息\"查看."));
    }
  } else {
    //结果和监视数量不一致
    auto running =
...
QString StationInfo::IP(QString niName) {
  auto iter = find_if(NetworkIntefaces.begin(), NetworkIntefaces.end(),
                      [&](NetworkInterface ni) {
...
QString StationInfo::toXmlString() {
  ...
  for (auto &app : getStartAppNames()) {
    QString appPath = QString::fromStdWString(app.AppPath);
...
void StationInfo::addStandAloneMonitorProcesses(
    const std::list<QString> &processes) {
...
void StationInfo::addStartApp(const QString &path, const QString &arguments,
  ...
  CC::AppStartParameter param{(int)startAppList.size(), path.toStdWString(),
                              arguments.toStdString(), procName.toStdString(),
                              allowMultiInstance};
...
void StationInfo::addStartApps(const std::list<CC::AppStartParameter> &apps) {
  for (auto &app : apps) {
    auto iter = find_if(startAppList.begin(), startAppList.end(),
                        [&app](CC::AppStartParameter &para) {
                          return para.AppPath == app.AppPath &&
                                 para.Arguments == app.Arguments &&
                                 para.ProcessName == app.ProcessName;
                        });
...
void StationInfo::clearStartApp() { startAppList.clear(); }

QStringList StationInfo::getStandAloneMonitorProcNames() {
  return standAloneMonitorProcessList;
}

void StationInfo::UpdateStation() { emit stationChanged(this); }
...
QString StationInfo::getStartAppNameByIndex(int ParamId) {
  for (auto &param : startAppList) {
    if (param.ParamId == ParamId)
      return QString::fromStdWString(param.AppPath);
  }
  ...
  for (auto &runningState : AppsRunningState) {
    if (runningState.isRunning) {
      auto app = find_if(startAppList.begin(), startAppList.end(),
                         [&](const CC::AppStartParameter param) {
                           return param.ProcessName ==
                                  runningState.Process.ProcessMonitorName;
                         });
...
*/
bool StationInfo::isSended(const QString &fileName,
                           const QDateTime &lastModify) {
  return std::any_of(sendedFiles.begin(), sendedFiles.end(),
                     [&](std::pair<QString, QDateTime> sendedFile) {
                       return sendedFile.first == fileName &&
                              sendedFile.second == lastModify;
                     });
}
...
*/
void StationInfo::addToSendedFileList(const QString &fileName,
                                      const QDateTime &lastModify) {
...
bool StationInfo::getHasErrorWhenSendFile() { return hasErrorWhenSendFile; }
...
std::list<QString> StationInfo::getSendFileLog() { return sendFileLogs; }

void StationInfo::setNetStatistics(const ::CC::Statistics &statistics) {
  this->currentStatistics = statistics;
  for (auto &ifData : statistics.IfStatistics) {
    for (int i = CounterHistoryDataSize - 1; i > 0; i--) {
      netStatisticsData[ifData.IfName][i] =
          netStatisticsData[ifData.IfName][i - 1];
    }
...
CC::Statistics StationInfo::getCurrentStatistics() { return currentStatistics; }
...
*/
void StationInfo::setAppsRunningState(
    const ::CC::AppsRunningState &appsRunningState) {
  this->AppsRunningState.clear();
  this->AppsRunningState.insert(this->AppsRunningState.begin(),
                                appsRunningState.begin(),
                                appsRunningState.end());
  for (auto &runningState : this->AppsRunningState) {
    if (runningState.isRunning) {
      if (counterData.find(runningState.Process.ProcessName) !=
          counterData.end()) {
        //移动数据
        for (int i = CounterHistoryDataSize - 1; i > 0; i--) {
          counterData[runningState.Process.ProcessName].cpuData[i] =
              counterData[runningState.Process.ProcessName].cpuData[i - 1];
          counterData[runningState.Process.ProcessName].memoryData[i] =
              counterData[runningState.Process.ProcessName].memoryData[i - 1];
        }
      }
      counterData[runningState.Process.ProcessName].cpuData[0] =
          runningState.cpu;
      counterData[runningState.Process.ProcessName].memoryData[0] =
          runningState.currentMemory / (double)TotalMemory() * 100;
    }
  }
}
...
*/
void StationInfo::subscribePropertyChanged(const QObject *receiver,
                                           const char *member) {
  connect(this, SIGNAL(propertyChanged(const QString &, const QObject *)),
          receiver, member);
}
...
*/
void StationInfo::subscribeStateChanged(const QObject *receiver,
                                        const char *member) {
...
创建时间：2015/11/13 10:55:42
*/
void StationInfo::subscribeStationChanged(const QObject *receiver,
                                          const char *member) {
...
*/
void StationInfo::State::setStatus(
    const StationInfo::OperatingStatus &status,
    QString attachMessage /*= QStringLiteral("")*/) {
...
QString StationInfo::State::toString() {
  ...
  return result.arg(translate(runningState))
      .arg(translate(operatingStatus))
      .arg(attachMessage);
}
...
StationInfo::RunningState StationInfo::State::getRunningState() {
  if (runningState == StationInfo::RunningState::Normal && blocked)
    return RunningState::NormalBlocked;
  else if (runningState == StationInfo::RunningState::Warning && blocked)
    return RunningState::WarningBlocked;
  return runningState;
}
...
bool StationInfo::getDownloadOption() { return downloadWeatherImage; }
...
QString StationInfo::State::toHtmlString() {
  switch (runningState) {
  case StationInfo::Unknown:
    return QStringLiteral("<span class=\"Unknown\">%1/</span>")
        .arg(this->toString());
    break;
  case StationInfo::Normal:
    return QStringLiteral("<span class=\"Normal\">%1/</span>")
        .arg(this->toString());
    break;
  case StationInfo::Warning:
    return QStringLiteral("<span class=\"Warning\">%1/</span>")
        .arg(this->toString());
    break;
  case StationInfo::Error:
    return QStringLiteral("<span class=\"Error\">%1/</span>")
        .arg(this->toString());
    break;
  default:
    break;
