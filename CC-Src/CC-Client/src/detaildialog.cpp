...
DetailDialog::DetailDialog(StationInfo *station, QWidget *parent)
  ...
  station->subscribeStateChanged(this,
                                 SLOT(stationStateChanged(const QObject *)));
  ...
  //总内存大小
  ui.memorySizeLineEdit->setText(
      QStringLiteral("%1").arg(station->TotalMemory() / 1024 / 1024));
    ...
    try {
      station->controlProxy->begin_getServerVersion(
          [=](const CC::ServerVersion &serverVersion) {
            if (!isClosed) {
              ui.servicesVersionLineEdit->setText(QString::fromStdString(
                  serverVersion.ServicesVersion.ProductVersion));
              ui.appProxyVersionLineEdit->setText(QString::fromStdString(
                  serverVersion.AppLuncherVersion.ProductVersion));
            }
          },
      ...
      if (!isClosed) {
        ui.servicesVersionLineEdit->setText(
            QStringLiteral("无法获取版本信息。"));
        ui.appProxyVersionLineEdit->setText(
            QStringLiteral("无法获取版本信息。"));
      }
    }
  }
...
void DetailDialog::on_powerOnPushButton_clicked() { manager->powerOn(station); }

void DetailDialog::on_powerOffPushButton_clicked() {
  manager->shutdown(station);
}

void DetailDialog::on_rebootPushButton_clicked() { manager->reboot(station); }
...
void DetailDialog::on_weatherImageDownloadCheckBox_clicked(bool checked) {
  ...
  if (prx != NULL) {
    Option::WeatherImageDownloadOption weatherImageDownloadOption =
        Option::getInstance()->weatherImageDownloadOption;
    ...
    ccOption.DeletePreviousFiles =
        weatherImageDownloadOption.DeletePreviousFiles;
    ccOption.DeleteHowHoursAgo = weatherImageDownloadOption.DeleteHowHoursAgo;
    ccOption.SubDirectory =
        weatherImageDownloadOption.SubDirectory.toStdWString();
    ccOption.SavePathForWindows =
        weatherImageDownloadOption.SavePathForWindows.toStdWString();
    ccOption.SavePathForLinux =
        weatherImageDownloadOption.SavePathForLinux.toStdWString();
    if (checked) {
      //下载气象云图
...
//工作站状态转化
void DetailDialog::stationStateChanged(const QObject *obj) { setButtonState(); }

void DetailDialog::closeEvent(QCloseEvent *event) { isClosed = true; }
...
void DetailDialog::timerEvent(QTimerEvent *e) {
  if (station != nullptr) {
    ...
    //更新总体信息
    ui.processCountLineEdit->setText(
        QStringLiteral("%1").arg(station->ProcCount()));
    ui.CPURateLineEdit->setText(QStringLiteral("%1").arg(station->CPU()));
    ui.memoryRateLineEdit->setText(QStringLiteral("%1").arg(
        station->Memory() * 100 / (float)station->TotalMemory()));
...
void DetailDialog::SetNetData() {
  ...
  ui.datagramsReceivedLineEdit->setText(
      QString("%1").arg(stat.DatagramsReceived));
  ui.datagramsSentlineEdit->setText(QString("%1").arg(stat.DatagramsSent));
  ui.datagramsDiscardedLineEdit->setText(
      QString("%1").arg(stat.DatagramsDiscarded));
  ui.datagramsWithErrorsLineEdit->setText(
      QString("%1").arg(stat.DatagramsWithErrors));
  ui.UDPListenersLineEdit->setText(QString("%1").arg(stat.UDPListeners));
  ui.SegmentsReceivedLineEdit->setText(
      QString("%1").arg(stat.SegmentsReceived));
  ...
  ui.CurrentConnectionsLineEdit->setText(
      QString("%1").arg(stat.CurrentConnections));
  ui.ResetConnectionscLineEdit->setText(
      QString("%1").arg(stat.ResetConnections));
  int ethCount = stat.IfStatistics.size();
  if (ethDataLineEdits.size() == ethCount * 10) {
    int index = 0;
    for (auto &ifData : stat.IfStatistics) {
      ethDataLineEdits[0 + index * 10]->setText(
          QString("%1").arg(ifData.BytesReceivedPerSec / 1024, 0, 'f', 3));
      ethDataLineEdits[1 + index * 10]->setText(
          QString("%1").arg(ifData.BytesSentedPerSec / 1024, 0, 'f', 3));
      ethDataLineEdits[2 + index * 10]->setText(
          QString("%1").arg(ifData.TotalBytesPerSec / 1024, 0, 'f', 3));
      ethDataLineEdits[3 + index * 10]->setText(
          QString("%1").arg(ifData.BytesReceived / 1024.0 / 1024.0, 0, 'f', 3));
      ethDataLineEdits[4 + index * 10]->setText(
          QString("%1").arg(ifData.BytesSented / 1024.0 / 1024.0, 0, 'f', 3));
      ethDataLineEdits[5 + index * 10]->setText(
          QString("%1").arg(ifData.BytesTotal / 1024.0 / 1024.0, 0, 'f', 3));
      ethDataLineEdits[6 + index * 10]->setText(
          QString("%1").arg(ifData.UnicastPacketReceived));
      ethDataLineEdits[7 + index * 10]->setText(
          QString("%1").arg(ifData.UnicastPacketSented));
      ethDataLineEdits[8 + index * 10]->setText(
          QString("%1").arg(ifData.MulticastPacketReceived));
      ethDataLineEdits[9 + index * 10]->setText(
          QString("%1").arg(ifData.MulticastPacketSented));
...
void DetailDialog::UpdateMonitoredProcessInfo() {
  ...
  for (auto &state : runningStates) {
    ...
    QTableWidgetItem *idItem =
        new QTableWidgetItem(QStringLiteral("%1").arg(state.Process.Id));
    ui.processTableWidget->setItem(row, 0, idItem);
    QTableWidgetItem *nameItem =
        new QTableWidgetItem(QString::fromStdString(state.Process.ProcessName));
    ui.processTableWidget->setItem(row, 1, nameItem);
    QTableWidgetItem *isRunningItem = new QTableWidgetItem(
        state.isRunning ? QStringLiteral("是") : QStringLiteral("否"));
    ui.processTableWidget->setItem(row, 2, isRunningItem);
    QTableWidgetItem *cpuItem =
        new QTableWidgetItem(QStringLiteral("%1").arg(state.cpu));
    ui.processTableWidget->setItem(row, 3, cpuItem);
    QTableWidgetItem *memoryItem =
        new QTableWidgetItem(QStringLiteral("%1").arg(
            state.currentMemory / (float)station->TotalMemory() * 100));
    ui.processTableWidget->setItem(row, 4, memoryItem);
    QTableWidgetItem *threadCountItem =
        new QTableWidgetItem(QStringLiteral("%1").arg(state.threadCount));
    ...
    if (finded != params.end()) {
      QString startPath = QString::fromStdWString(finded->AppPath);
...
void DetailDialog::addEthGroup(int index, QString ifName) {
  ...
  ethGroupBox->setTitle(
      QStringLiteral("%1(%2) 统计数据").arg(ifName).arg(station->IP(ifName)));
  ...
  bytesReceivedPerSecLineEdit->setObjectName(
      QStringLiteral("bytesReceivedPerSecLineEdit"));
  ...
  bytesSentedLineEdit->setObjectName(
      QStringLiteral("bytesSentPerSeclineEdit_3"));
  ...
  bytesTotalLineEdit->setObjectName(
      QStringLiteral("bytesTotalPerSecLineEdit_3"));
