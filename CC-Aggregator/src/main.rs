...
async fn main() -> Result<()> {
    ...
    // Load configuration
    let config = AggregatorConfig::load(&args.config).unwrap_or_else(|e| {
    ...
    // Create MQTT client
    let mqtt_client = MqttClient::new(&config.mqtt.host, config.mqtt.port, &config.mqtt.client_id)?;
    ...
    let processor_handle = tokio::spawn(async move {
        let mut receiver: broadcast::Receiver<mqtt::MqttMessage> =
            ws_server_for_processor.subscribe_to_messages();
                ...
                match msg_type.as_str() {
                    "telemetry" => {
                        if let Some(telemetry) = msg.parse_json::<telemetry::TelemetryBundle>() {
                            ws_server_for_processor
                                .broadcast_telemetry(&station_id, &telemetry)
                                .await;
                        }
                    }
                    "status" => {
                        if let Some(status) = msg.parse_json::<mqtt::StationStatus>() {
                            ws_server_for_processor
                                .broadcast_status(&station_id, &status)
                                .await;
                        }
                    }
                    _ => {}
